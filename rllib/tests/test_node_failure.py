...
class NodeFailureTests(unittest.TestCase):
    def setUp(self):
        ...
        ray.init(address=self.cluster.address, local_mode=False)
    ...
    def test_continue_training_on_failure(self):
        ...
        self.assertEqual(ppo.workers.num_healthy_workers(), 6)
        ...
        self.assertEqual(ppo.workers.num_healthy_workers(), 4)
        self.assertEqual(ppo.workers.num_remote_workers(), 6)
        ...
        while True:
            states = [
                a["state"] == 'ALIVE' for a in list_actors()
