...
class FaultTolerantActorManager:
    ...
    def __call_actors(
        ...
        *,
        ...
        if funcs:
            assert len(remote_actor_indices) == len(funcs), (
        ...
        if func:
            calls = [
                self.__actors[i].apply.remote(func) for i in remote_actor_indices
            ]
        else:
            calls = [
    ...
    def __fetch_result_and_mark_state(
        ...
        *,
        ...
        ready, _ = ray.wait(
            ...
            timeout=timeout,
            fetch_local=True,  # Make sure remote results are fetched locally in parallel.
        )
    ...
    def foreach_actor(
        ...
        *,
        ...
        remote_actor_indices = (
            remote_actor_indices or list(range(len(self.__actors)))
        )
        if healthy_only:
            remote_actor_indices = [
        ...
        if self._ignore_worker_failure:
            ...
            remote_results = list(
                filter(lambda r: not isinstance(r, RayError), remote_results)
            )
    ...
    def foreach_actor_async(
        ...
        *,
        ...
        remote_actor_indices = [
            i for i in remote_actor_indices
    ...
    def fetch_ready_async_reqs(
        ...
        *,
        ...
        for call, result in zip(ready, remote_results):
            ...
            if not isinstance(
                result,
                Exception if self._ignore_worker_failure else RayError
            ):
